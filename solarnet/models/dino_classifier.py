...
class DinoFinegrainedClassifier(nn.Module):
    ...
    def __init__(self, layers=1, mode='small', stacked_layers=1) -> None:
        ...
        if mode == 'small':
            ...
            self.dinov2 = torch.hub.load('facebookresearch/dinov2', 'dinov2_vitg14')
            self.linear_head = nn.Linear(1536, 1)
    ...
    def forward(self, x):
        ...
        patch_logits = self.linear_head(patch_tokens)
        patch_logits = torch.log(F.sigmoid(patch_logits))
        neg_logit = torch.sum(patch_logits, dim=1)
        pos_prob = 1 - torch.exp(neg_logit)
        return pos_prob