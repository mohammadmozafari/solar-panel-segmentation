...
from solarnet.models import (Classifier, Segmenter, train_classifier, train_segmenter,
                             DinoClassifier, DinoFinegrainedClassifier, train_classifier_single_output)
...
class RunTask:
    ...
    @staticmethod
    def train_finegrained_classifier(max_epochs=100, warmup=2, patience=5, val_size=0.1,
                                     test_size=0.1, data_folder='data', mode='small', train_mode='freeze',
                                     device=torch.device('cuda:0' if torch.cuda.is_available() else 'cpu'),
                                     exp_dir=None, exp_name=None):
        ...
        model = DinoFinegrainedClassifier(mode=mode, layers=1, stacked_layers=2)
        ...
        train_classifier_single_output(model, train_dataloader, sub_val_dataloader, max_epochs=max_epochs,
                                       warmup=warmup, patience=patience, device=device,
                                       instance_dir=instance_dir, writer=writer, learning_rate=learning_rate)
